...
class test extends CI_Controller {
	...
	public function __construct(){
		...
		$this->mhs 	= $this->test->getIdMahasiswa($this->user->username);
    }
    ...
    public function master()
	{
        ...
        $data = [
			...
			'judul'	=> 'Exam',
			'subjudul'=> 'Exam Data',
			'dosen' => $this->test->getIdDosen($user->username),
		];
	...
	public function validasi()
	{
		...
		$user 	= $this->ion_auth->user()->row();
		$dosen 	= $this->test->getIdDosen($user->username);
		$jml 	= $this->test->getJumlahSoal($dosen->id_dosen)->jml_soal;
		...
		$this->form_validation->set_rules('nama_test', 'Exam Name', 'required|alpha_numeric_spaces|max_length[50]');
	...
	public function save()
	{
		...
		$nama_test 	= $this->input->post('nama_test', true);
		...
		if( $this->form_validation->run() === FALSE ){
			$data['status'] = false;
			$data['errors'] = [
				'nama_test' 	=> form_error('nama_test'),
			...
			if($method === 'add'){
				...
				$input['token']		= $token;
				$action = $this->master->create('m_test', $input);
			}else if($method === 'edit'){
				$id_test = $this->input->post('id_test', true);
				$action = $this->master->update('m_test', $input, 'id_test', $id_test);
			}
	...
	public function delete()
	{
        ...
        if(!$chk){
            ...
            if($this->master->delete('m_test', $chk, 'id_test')){
	...
	public function refresh_token($id)
	{
		...
		$refresh = $this->master->update('m_test', $data, 'id_test', $id);
	...
	public function list_json()
	{
		...
		$list = $this->test->getListtest($this->mhs->id_mahasiswa, $this->mhs->kelas_id);
	...
	public function list()
	{
		...
		$data = [
			...
			'judul'		=> 'Exam',
			'subjudul'	=> 'List Exam',
			'mhs' 		=> $this->test->getIdMahasiswa($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	{
		$id = $this->input->post('id_test', true);
	...
	public function index()
	{
		...
		$test 		= $this->test->gettestById($id);
		$soal 		= $this->test->getSoal($id);
		
		$mhs		= $this->mhs;
		$h_test 	= $this->test->Hsltest($id, $mhs->id_mahasiswa);
	
		$cek_sudah_ikut = $h_test->num_rows();
		...
		if ($cek_sudah_ikut < 1) {
			...
			$waktu_selesai 	= date('Y-m-d H:i:s', strtotime("+{$test->waktu} minute"));
			...
			$this->master->create('h_test', $input);
		...
		for ($i = 0; $i < sizeof($urut_soal); $i++) {
			...
			$ambil_soal 	= $this->test->ambilSoal($pc_urut_soal1, $pc_urut_soal[0]);
	...
	public function simpan_satu()
	{
		...
		// Simpan jawaban
		$this->master->update('h_test', $d_simpan, 'id', $id_tes);
	...
	public function simpan_akhir()
	{
		...
		// Get Jawaban
		$list_jawaban = $this->test->getJawaban($id_tes);
		...
		$this->master->update('h_test', $d_update, 'id', $id_tes);
